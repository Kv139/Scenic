...
class WebotsSimulation(Simulation):
    ...
    def get_coverage_reward(self, pos):
        ...
        if len(self.covered_spaces) / self.total_spaces > self.checkpoint: # every 10% of the room that is covered gets larger reward
            reward += 10
            self.checkpoint += .1
    ...
    def get_reward(self): # "any dummy for now will be okay"
        ...
        if reward == 0: # no exploration small penalization
            reward += -1 

        if self.invalid_action: # invalid action in the space large penalization
            reward = -10
            self.invalid_action = False
        ...
        if (np.any(self.observation[2:] < 0.7) ): # if any distance sensor is low penalize
